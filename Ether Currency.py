...
def ethereum_block():
    wei = web3.eth.gasPrice
    Gwei = wei/10**9

    ether = Gwei/10**9
    etherDollar = ether * 1763.43
    etherRuppees = ether * 136850.54
    etherPound = ether * 1393.29

    value_in_ether['text'] = 'Value of Ether Gas - ' + str(ether) + 'ETH'
    value_in_dollar['text'] = 'Value of Ether($) - ' + f'${str(etherDollar)}'
    value_in_rupees['text'] = 'Value of Ether(₹) - ' + f'₹{str(etherRuppees)}'
    value_in_pounds['text'] = 'Value of Ether(£) - ' + f'£{str(etherPound)}'
